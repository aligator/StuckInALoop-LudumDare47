...
public class BodyEditorLoader {
     ...
     */
    public void attachFixture(Body body, String name, FixtureDef fd, float scaleX, float scaleY, boolean flipX) {
        ...
        for (int i = 0, n = rbModel.circles.size(); i < n; i++) {
            ...
            center.x *= scaleX;
            center.y *= scaleY;
            float radius = circle.radius * Math.max(scaleX, scaleY);
    ...
    public Vector2 getOrigin(String name, float scale) {
        ...
        Vector2 v = vec.set(rbModel.origin);
        v.x *= scale;
        v.y *= scale;
        return v;
    }
